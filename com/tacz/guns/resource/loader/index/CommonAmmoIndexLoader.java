...
/*    */ {
/* 31 */   private static final Pattern AMMO_INDEX_PATTERN = Pattern.compile("^(\\w+)/ammo/index/(\\w+)\\.json$");
/* 32 */   private static final Marker MARKER = MarkerManager.getMarker("CommonAmmoIndexLoader");
/*    */   
/*    */   public static void loadAmmoIndex(String path, ZipFile zipFile) throws IOException {
/* 35 */     Matcher matcher = AMMO_INDEX_PATTERN.matcher(path);
/* 36 */     if (matcher.find()) {
...
/* 47 */           loadAmmoFromJsonString(registryName, json);
/* 48 */           CommonGunPackNetwork.addData(DataType.AMMO_INDEX, registryName, json);
...
/* 57 */     Path filePath = root.toPath().resolve("ammo/index");
/* 58 */     if (Files.isDirectory(filePath, new java.nio.file.LinkOption[0])) {
/* 59 */       TacPathVisitor visitor = new TacPathVisitor(filePath.toFile(), root.getName(), ".json", (id, file) -> { try { InputStream stream = Files.newInputStream(file, new java.nio.file.OpenOption[0]); try { String json = IOUtils.toString(stream, StandardCharsets.UTF_8); loadAmmoFromJsonString(id, json); CommonGunPackNetwork.addData(DataType.AMMO_INDEX, id, json); if (stream != null)
...
/* 74 */     AmmoIndexPOJO indexPOJO = (AmmoIndexPOJO)CommonGunPackLoader.GSON.fromJson(json, AmmoIndexPOJO.class);
/* 75 */     CommonGunPackLoader.AMMO_INDEX.put(id, CommonAmmoIndex.getInstance(indexPOJO));
/*    */   }
/*    */ }


/* Location:              C:\Users\gabri\AppData\Roaming\.minecraft\mods\tacz-1.20.1-1.0.3-all.jar!\com\tacz\guns\resource\loader\index\CommonAmmoIndexLoader.class
