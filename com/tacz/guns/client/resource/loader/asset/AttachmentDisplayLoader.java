...
/*    */ {
/* 27 */   private static final Marker MARKER = MarkerManager.getMarker("AttachmentDisplayLoader");
/* 28 */   private static final Pattern DISPLAY_PATTERN = Pattern.compile("^(\\w+)/attachments/display/([\\w/]+)\\.json$");
/*    */   
...
/* 42 */           AttachmentDisplay display = (AttachmentDisplay)ClientGunPackLoader.GSON.fromJson(new InputStreamReader(stream, StandardCharsets.UTF_8), AttachmentDisplay.class);
/* 43 */           ClientAssetManager.INSTANCE.putAttachmentDisplay(registryName, display);
...
/* 54 */     Path displayPath = root.toPath().resolve("attachments/display");
/* 55 */     if (Files.isDirectory(displayPath, new java.nio.file.LinkOption[0])) {
/* 56 */       TacPathVisitor visitor = new TacPathVisitor(displayPath.toFile(), root.getName(), ".json", (id, file) -> { try { InputStream stream = Files.newInputStream(file, new java.nio.file.OpenOption[0]); try { AttachmentDisplay display = (AttachmentDisplay)ClientGunPackLoader.GSON.fromJson(new InputStreamReader(stream, StandardCharsets.UTF_8), AttachmentDisplay.class); ClientAssetManager.INSTANCE.putAttachmentDisplay(id, display); if (stream != null)
...
/* Location:              C:\Users\gabri\AppData\Roaming\.minecraft\mods\tacz-1.20.1-1.0.3-all.jar!\com\tacz\guns\client\resource\loader\asset\AttachmentDisplayLoader.class
