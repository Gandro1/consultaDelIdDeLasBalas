...
/*    */ {
/* 27 */   private static final Marker MARKER = MarkerManager.getMarker("AttachmentSkinLoader");
/* 28 */   private static final Pattern SKIN_PATTERN = Pattern.compile("^(\\w+)/attachments/skin/([\\w/]+)\\.json$");
/*    */   
/*    */   public static boolean load(ZipFile zipFile, String zipPath) {
/* 31 */     Matcher matcher = SKIN_PATTERN.matcher(zipPath);
/* 32 */     if (matcher.find()) {
...
/* 42 */           AttachmentSkin display = (AttachmentSkin)ClientGunPackLoader.GSON.fromJson(new InputStreamReader(stream, StandardCharsets.UTF_8), AttachmentSkin.class);
/* 43 */           ClientAssetManager.INSTANCE.putAttachmentSkin(registryName, display);
...
/* 46 */       { GunMod.LOGGER.warn(MARKER, "Failed to read skin display file: {}, entry: {}", zipFile, entry);
...
/* 54 */     Path displayPath = root.toPath().resolve("attachments/skin");
/* 55 */     if (Files.isDirectory(displayPath, new java.nio.file.LinkOption[0])) {
/* 56 */       TacPathVisitor visitor = new TacPathVisitor(displayPath.toFile(), root.getName(), ".json", (id, file) -> { try { InputStream stream = Files.newInputStream(file, new java.nio.file.OpenOption[0]); try { AttachmentSkin display = (AttachmentSkin)ClientGunPackLoader.GSON.fromJson(new InputStreamReader(stream, StandardCharsets.UTF_8), AttachmentSkin.class); ClientAssetManager.INSTANCE.putAttachmentSkin(id, display); if (stream != null)
...
/* Location:              C:\Users\gabri\AppData\Roaming\.minecraft\mods\tacz-1.20.1-1.0.3-all.jar!\com\tacz\guns\client\resource\loader\asset\AttachmentSkinLoader.class
